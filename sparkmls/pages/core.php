...
class flexmlsConnectPageCore {
  ...
  public function parse_search_parameters_into_api_request($search_arr=array()) {
    global $fmc_api;

    // pull StandardFields from the API to verify searchability prior to searching [idx_listing_summary link="rmnx2n9czyw" source="location" display="all" sort="recently_changed" status="Active" DaysOnMarket="7"]
    $result = $fmc_api->GetStandardFields();
   
    // $aa = $fmc_api->GetStandardField('MajorChangeType');
    //  echo "<pre>"; print_r($aa);    
    // die;
    ...
    $catch_fields = array( 
      array(
        'input' => 'MajorChangeType',
        'operator' => 'Eq',
        'field' => 'MajorChangeType'
      ),
      array(
    ...
    foreach ($catch_fields as $f) { 
      if(($f['input'] == 'SavedSearch' || $f['input'] == 'OnMarketDate') && !empty($_GET['alaskaSearch']))
      {
          continue;
      }
      if(!empty($search_arr))
      {
          ...
          if (array_key_exists($f['input'], $search_arr)) {
            if($f['input'] != 'City')
            {
              $f['field'] = $f['field'];
              
                $value = $search_arr[$f['input']];
    ...
    if(($key = array_search('GarageSpaces Eq 0.00', $search_criteria)) !== false) {
    ...
    $limit = ($desired_limit) ? $desired_limit : 12;
    if ($limit != 12) {
      $cleaned_raw_criteria['Limit'] = $limit;
    }

    if(array_key_exists("MinMajorChangeTimestamp",$search_arr) && array_key_exists("MaxMajorChangeTimestamp",$search_arr)) 
    {
       $search_criteria_part = "MajorChangeTimestamp Bt ".$search_arr['MinMajorChangeTimestamp'].",".$search_arr['MaxMajorChangeTimestamp'];
       array_push($search_criteria, $search_criteria_part);
       $cleaned_raw_criteria[ 'MajorChangeTimestamp' ] = $search_arr['MinMajorChangeTimestamp'];
       $cleaned_raw_criteria[ 'MajorChangeTimestamp' ] = $search_arr['MaxMajorChangeTimestamp'];
    }

    if(($key = array_search('()', $search_criteria)) !== false) {
        unset($search_criteria[$key]);
    }

    if(($key = array_search("(City Eq '')", $search_criteria)) !== false) {
        unset($search_criteria[$key]);
    }

   

    if(array_key_exists("City",$search_arr)) 
    {
      $cleaned_raw_criteria[ 'City' ] = $search_arr['City'];
      $search_criteria_part .= '(';
      foreach ($search_arr['City'] as $value) {        
        $search_criteria_part_2[] = "City Eq '".$value."'";        
      }
      $search_criteria_part .= implode(" Or ", $search_criteria_part_2);
      $search_criteria_part .= ')';
      array_push($search_criteria, $search_criteria_part);
    }  
    ...
    if(!empty($_GET['alaskaSearch']))
    {
        $cleaned_raw_criteria[ 'alaskaSearch' ] = $_GET['alaskaSearch'];
        //array_push($search_criteria, "(SavedSearch Eq '20170427164825001017000000' Or SavedSearch Eq '20170427165216949677000000' Or SavedSearch Eq  '20170427165103666489000000')");
        
          
    }
    //array_push($search_criteria, "(SavedSearch Eq '20101018074152570937000000')");
    
    if(empty($_GET['StandardStatus']) && $_GET['StandardStatus'] == 0 && !empty($_GET['alaskaSearch']))
    {        
      // if(($key = array_search("(StandardStatus Eq 'Active') )", $search_criteria)) !== false) {
      //     unset($search_criteria[$key]);
      // }
      if(($key = array_search("(StandardStatus Eq 'Active')", $search_criteria)) !== false) {
          unset($search_criteria[$key]);
    ...
    if(empty($_GET['PropertyType']) && $_GET['PropertyType'] == 0)
    {     
      
      if(($key = array_search("(PropertyType Eq '0')", $search_criteria)) !== false) {
          unset($search_criteria[$key]);
      }
    }
    //echo count($search_criteria);

    if(count($search_criteria) == 1 && strpos($search_criteria[0], 'StandardStatus Eq') !== false && empty($_GET['StandardStatus']))
    {
      $search_criteria = array();
      unset($cleaned_raw_criteria[ 'StandardStatus' ]);
    }
