...
class flexmlsConnectPage {
  ...
  static function do_rewrite() {
    ...
    add_rewrite_rule( 
      $options['permabase'] .'/([^/]+)?' , 
      'index.php?plugin=sparkmls&fmc_tag=$matches[1]&page_id='. $options['destlink'] , 
      'top' 
    );
    add_rewrite_rule(
      'portal'.'/([^/]+)?',
      'index.php?plugin=sparkmls&fmc_vow_tag=$matches[1]&page_id='. $options['destlink'],
      'top' 
    );
    add_rewrite_rule('oauth/callback', 'index.php?plugin=sparkmls&oauth_tag=oauth-login', 'top');
  }
