	...
	static function wp_enqueue_scripts(){
		...
		if( defined( 'FMC_DEV' ) && FMC_DEV ){
        	...
        	if ($current_user->ID > 0) {
				wp_enqueue_script( 'fmc_connect', plugins_url( 'assets/minified/connect.min.js?v='.time(), dirname( __FILE__ ) ), '1.0.0', false );
			}else{
				wp_enqueue_script( 'fmc_connect', plugins_url( 'assets/minified/connect.min.js?v='.time(), dirname( __FILE__ ) ), '1.0.0', false );
			}			
			wp_enqueue_script( 'fmc_portal', plugins_url( 'assets/minified/portal.min.js', dirname( __FILE__ ) ), array( 'jquery', 'fmc_connect' ), FMC_PLUGIN_VERSION, true );
		}
		wp_localize_script( 'fmc_connect', 'fmcAjax', array(
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'pluginurl' => plugins_url( '', dirname( __FILE__ ) )
		...
		wp_enqueue_script( 'common_js', plugins_url( 'assets/minified/common.js?v='.time(), dirname( __FILE__ ) ), time(), true );

		wp_enqueue_style( 'wp-jquery-ui-dialog' );
		wp_enqueue_style( 'fmc_connect', plugins_url( 'assets/css/style.css?v='.time(), dirname( __FILE__ ) ), FMC_PLUGIN_VERSION );
